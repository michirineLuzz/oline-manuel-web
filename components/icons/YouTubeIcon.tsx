
import React from 'react';

const YouTubeIcon: React.FC = () => (
  <svg xmlns="https://www.google.com/url?sa=i&url=https%3A%2F%2Fid.m.wikipedia.org%2Fwiki%2FBerkas%3AIDN_App.svg&psig=AOvVaw1nWzXpzK3uU_NwsC4fZT2v&ust=1761650324776000&source=images&cd=vfe&opi=89978449&ved=0CBUQjRxqFwoTCKiblIGhxJADFQAAAAAdAAAAABAE" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path>
    <path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
  </svg>
);

export default YouTubeIcon;
